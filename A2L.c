...
void A2lSetEvent(unsigned short event) {
	gA2lEvent = event;
}
...
void A2lCreateTypedefInstance_(const char* instanceName, const char* typeName, vuint32 addr, const char* comment) {
	fprintf(gA2lFile, "/begin INSTANCE %s \"%s\" %s 0x%X", instanceName, comment, typeName, (vuint32)addr);
	if (gA2lEvent >= 0) {
...
void A2lCreateMeasurement_(const char* instanceName, const char* name, int size, vuint32 addr, double factor, double offset, const char* unit, const char* comment) {
	...
	if (instanceName) {
		fprintf(gA2lFile, "/begin MEASUREMENT %s.%s \"%s\" %s %s_COMPU_METHOD 0 0 %s %s ECU_ADDRESS 0x%X", instanceName, name, comment, getMeaType(size), conv, getTypeMin(size), getTypeMax(size), (vuint32)addr);
	}
	else {
		fprintf(gA2lFile, "/begin MEASUREMENT %s \"%s\" %s %s_COMPU_METHOD 0 0 %s %s ECU_ADDRESS 0x%X", name, comment, getMeaType(size), conv, getTypeMin(size), getTypeMax(size), (vuint32)addr);
	}
...
void A2lCreateMeasurementArray_(const char* instanceName, const char* name, int size, int dim, vuint32 addr) {

	if (instanceName) {
		fprintf(gA2lFile, "/begin CHARACTERISTIC %s.%s \"\" VAL_BLK 0x%X %s 0 NO_COMPU_METHOD %s %s MATRIX_DIM %u", instanceName, name, (vuint32)addr, getParType(size), getTypeMin(size), getTypeMax(size), dim);
	}
	else {
		fprintf(gA2lFile, "/begin CHARACTERISTIC %s \"\" VAL_BLK 0x%X %s 0 NO_COMPU_METHOD %s %s MATRIX_DIM %u", name, (vuint32)addr, getParType(size), getTypeMin(size), getTypeMax(size), dim);
	}
	if (gA2lEvent>=0) {
		fprintf(gA2lFile," /begin IF_DATA XCP /begin DAQ_EVENT FIXED_EVENT_LIST EVENT 0x%X /end DAQ_EVENT /end IF_DATA", gA2lEvent);
	}
...
void A2lCreateParameterWithLimits_(const char* name, int size, vuint32 addr, const char* comment, const char* unit, double min, double max) {
...
void A2lCreateParameter_(const char* name, int size, vuint32 addr, const char* comment, const char* unit) {
...
void A2lCreateMap_(const char* name, int size, vuint32 addr, unsigned int xdim, unsigned int ydim, const char* comment, const char* unit) {
...
void A2lClose(void) {
	...
	fclose(gA2lFile);
	printf("Created A2L %s\n  (%u meas, %u pars, %u typedefs, %u components, %u instances, %u conversions)\n",
		gA2lFilename, gA2lMeasurements, gA2lParameters, gA2lTypedefs, gA2lComponents, gA2lInstances, gA2lConversions);
}
