...
#define A2lGetOffset_(var) (uint16_t)((uint8_t*)&(var) - (uint8_t*)this) // object data size limited to 64K !
#define A2lGetAddr_(var) Xcp::getInstance()->getA2lAddr((uint8_t*)&var)

// "createDynXxx" functions are assumed to be called from methods of classes derived from XcpObject !
#define A2lGetDynAddr_(var) xcpGetA2lDynAddr(A2lGetOffset_(var)) // inside XcpObject
#define A2lGetDynBaseAddr_() xcpGetA2lDynAddr(0) // inside XcpObject
...
#define createTypedefParameterComponent(name)                                       createTypedefParameterComponent_(#name,A2lGetType_(name),A2lGetOffset_(name))
#define createTypedefMeasurementComponent(name)                                     createTypedefMeasurementComponent_(#name,A2lGetType_(name),A2lGetOffset_(name))
