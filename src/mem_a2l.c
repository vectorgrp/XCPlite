...
#define A2L_INTERNAL_COMPILATION
#include "a2l.h"
...
static uint8_t gA2lMode = 0; // Mode for A2L generation,
...
static bool gA2lFinalized = false;
...
static a2l_object_list_t *gA2lObjectList = NULL; // Binary representation of the A2L description

//----------------------------------------------------------------------------------
static const char *gA2lObjectListText = "ASAP2_VERSION 1 71\n"
                                        "/begin PROJECT %s \"\"\n\n"
...
static bool A2lOpen(const char *filename, const char *projectname) {
    ...
    // Create headers
    fprintf(gA2lFile, gA2lObjectListText, projectname, projectname); // main file
...
// Set segment relative address mode with calibration segment index
...
bool A2lOnce_(uint64_t *value) {
    ...
    if (atomic_compare_exchange_strong_explicit(value, &old_value, 1, memory_order_relaxed, memory_order_relaxed)) {
        return true;
    }
    return false;
}
...
static bool includeA2lFile(FILE **filep, const char *filename) {
...
static void finalizeA2lFile(void) {
    ...
    includeA2lFile(&gA2lTypedefsFile, "typedefs.a2l");
    includeA2lFile(&gA2lGroupsFile, "groups.a2l");
    includeA2lFile(&gA2lConversionsFile, "conversions.a2l");
    ...
    DBG_PRINTF3("A2L file %s created: %u measurements, %u params, %u typedefs, %u components, %u instances, %u conversions\n", gA2lFilename, gA2lMeasurements, gA2lParameters,
                gA2lTypedefs, gA2lComponents, gA2lInstances, gA2lConversions);
}
...
bool A2lFinalize(void) {
    ...
    // If A2l file is open (OPTION_A2L_WRITER), finalize it
    if (gA2lFile != NULL) {

        finalizeA2lFile(); // Finalize the A2L file
    ...
    // If object list is not NULL (OPTION_A2L_CREATOR),
    else if (gA2lObjectList != NULL) {

        // @@@@ TODO: Implement
        // Provide object list for upload to XCP
        DBG_PRINTF3("A2L object list ready, %u objects\n", gA2lObjectList->count);
        return true;
    }
...
// Lock and unlock
void A2lLock(void) {
    assert(gA2lFile != NULL || gA2lMode == A2L_MODE_IN_MEMORY);
    mutexLock(&gA2lMutex);
}
...
bool A2lInit(const char *a2l_projectname, const char *a2l_version, const uint8_t *addr, uint16_t port, bool useTCP, uint8_t mode) {
    ...
    // Save mode
    gA2lMode = mode & (A2L_MODE_WRITE_ONCE | A2L_MODE_WRITE_ALWAYS);
    ...
    if (mode & (A2L_MODE_WRITE_ONCE | A2L_MODE_WRITE_ALWAYS)) {
        ...
        DBG_PRINTF3("Start A2L generator, file=%s, mode=%u, finalize_on_connect=%u, auto_groups=%u\n", gA2lFilename, mode & (A2L_MODE_WRITE_ONCE | A2L_MODE_WRITE_ALWAYS),
...
//-----------------------------------------------------------------------------------------------------
// A2L Creator: Binary A2L representation

// Create a new A2L object of type tag with the given metadata
// @@@@ TODO: Make this thread safe
bool A2lCreateObject(uint8_t tag, uint8_t addr_ext, uint16_t event_id, uint32_t addr, const void *metadata) {

    assert(gA2lMode == A2L_MODE_IN_MEMORY);
    assert(!gA2lFinalized);

    // Initialize on first use
    if (gA2lObjectList == NULL) {
        gA2lObjectList = (a2l_object_list_t *)malloc(sizeof(a2l_object_list_t));
        if (gA2lObjectList == NULL) {
            DBG_PRINT_ERROR("A2lCreateObject: Failed to allocate memory for A2L header\n");
            return false;
        }
        gA2lObjectList->version = (OPTION_VERSION_MAJOR << 16) | (OPTION_VERSION_MINOR << 8) | (OPTION_VERSION_PATCH);
        gA2lObjectList->count = 0;
    }

    // Check if we have space for another object
    if (gA2lObjectList->count >= A2L_MAX_OBJECTS) {
        DBG_PRINTF_WARNING("A2lCreateObject: Maximum number of A2L objects (%u) reached, object ignored\n", A2L_MAX_OBJECTS);
        return false;
    }

    // Add the object to the global header structure
    a2l_object_t *obj = &gA2lObjectList->objects[gA2lObjectList->count];
    obj->tag = tag;
    obj->addr_ext = addr_ext;
    obj->event_id = event_id;
    obj->addr = addr;
    obj->metadata = (void *)metadata;

    gA2lObjectList->count++;

    DBG_PRINTF3("A2lCreateObject: %u: tag=%u, addr=%u:0x%08X, event_id=%u\n", gA2lObjectList->count, tag, addr_ext, addr, event_id);

    return true;
}

// Get access to the global A2L object list
const a2l_object_list_t *A2lGetHeader(void) {
    assert(gA2lMode == A2L_MODE_IN_MEMORY);
    return gA2lObjectList;
}

// Generate A2l from the static data structure

// Print the A2L header information for debugging
void A2lPrintObjectList(void) {
    if (gA2lObjectList == NULL) {
        printf("A2lPrintHeader: gA2lObjectList is NULL\n");
        return;
    }

    printf("=== A2L Header Debug Information ===\n");
    printf("Version: %u\n", gA2lObjectList->version);
    printf("Object Count: %u\n", gA2lObjectList->count);
    printf("\n");

    if (gA2lObjectList->count == 0) {
        printf("No objects in A2L header\n");
        return;
    }

    printf("Objects:\n");
    for (uint32_t i = 0; i < gA2lObjectList->count; i++) {
        const a2l_object_t *obj = &gA2lObjectList->objects[i];

        // Determine object type from tag and dimensions
        const char *type_str;
        switch (obj->tag) {
        case A2L_OBJECT_TYPE_CONVERSION:
            type_str = "CONVERSION";
            break;
        case A2L_OBJECT_TYPE_PARAMETER: {
            // Determine parameter subtype based on dimensions
            const a2l_parameter_t *param = (const a2l_parameter_t *)obj->metadata;
            if (param != NULL) {
                if (param->y_dim_ > 1) {
                    type_str = "PARAMETER (MAP)";
                } else if (param->x_dim_ > 1 && param->y_dim_ == 1) {
                    type_str = "PARAMETER (CURVE)";
                } else {
                    type_str = "PARAMETER (SCALAR)";
                }
            } else {
                type_str = "PARAMETER";
            }
        ...
        case A2L_OBJECT_TYPE_MEASUREMENT: {
            // Determine measurement subtype based on dimensions
            const a2l_measurement_t *meas = (const a2l_measurement_t *)obj->metadata;
            if (meas != NULL) {
                if (meas->y_dim_ > 1) {
                    type_str = "MEASUREMENT (MATRIX)";
                } else if (meas->x_dim_ > 1 && meas->y_dim_ == 1) {
                    type_str = "MEASUREMENT (ARRAY)";
                } else {
                    type_str = "MEASUREMENT (SCALAR)";
                }
            } else {
        ...
        printf("  [%u] Type: %s (tag=%u)\n", i, type_str, obj->tag);
        printf("      Address Extension: %u (decimal), 0x%02X (hex)\n", obj->addr_ext, obj->addr_ext);
        printf("      Event ID: %u\n", obj->event_id);
        printf("      Address: 0x%08X\n", obj->addr);
        printf("      Metadata: %p\n", obj->metadata);

        // Print detailed metadata based on object type
        if (obj->metadata != NULL) {
            printf("      Metadata Details:\n");
            switch (obj->tag) {
            case A2L_OBJECT_TYPE_CONVERSION: {
                const a2l_conversion_t *conv = (const a2l_conversion_t *)obj->metadata;
                printf("        Name: %s\n", conv->name_ ? conv->name_ : "(null)");
                printf("        Comment: %s\n", conv->comment_ ? conv->comment_ : "(null)");
                printf("        Unit: %s\n", conv->unit_ ? conv->unit_ : "(null)");
                printf("        Description: %s\n", conv->description_ ? conv->description_ : "(null)");
                printf("        Is Linear: %s\n", conv->is_linear_ ? "Yes" : "No");
                if (conv->is_linear_) {
                    printf("        Factor: %f\n", conv->factor_);
                    printf("        Offset: %f\n", conv->offset_);
                }
            ...
            case A2L_OBJECT_TYPE_PARAMETER: {
                const a2l_parameter_t *param = (const a2l_parameter_t *)obj->metadata;
                printf("        Name: %s\n", param->name_ ? param->name_ : "(null)");
                printf("        Comment: %s\n", param->comment_ ? param->comment_ : "(null)");
                printf("        Unit: %s\n", param->unit_ ? param->unit_ : "(null)");
                printf("        X Axis: %s\n", param->x_axis_ ? param->x_axis_ : "(null)");
                printf("        Y Axis: %s\n", param->y_axis_ ? param->y_axis_ : "(null)");
                printf("        Min: %f\n", param->min_);
                printf("        Max: %f\n", param->max_);
            ...
            case A2L_OBJECT_TYPE_MEASUREMENT: {
                const a2l_measurement_t *meas = (const a2l_measurement_t *)obj->metadata;
                printf("        Name: %s\n", meas->name_ ? meas->name_ : "(null)");
                printf("        Comment: %s\n", meas->comment_ ? meas->comment_ : "(null)");
                printf("        Unit: %s\n", meas->unit_ ? meas->unit_ : "(null)");
                printf("        Min: %f\n", meas->min_);
                printf("        Max: %f\n", meas->max_);
                printf("        X Dimension: %u\n", meas->x_dim_);
                printf("        Y Dimension: %u\n", meas->y_dim_);
            ...
            default:
                printf("        (Unknown metadata format for type %u)\n", obj->tag);
                break;
            }
        } else {
            printf("      Metadata Details: (null)\n");
        }

        printf("\n");
    }
    printf("=== End A2L Header Debug Information ===\n");
}
