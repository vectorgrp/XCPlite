...
static bool XcpEthTlSend(const uint8_t *data, uint16_t size, const uint8_t *addr, uint16_t port) {
...
#ifdef XCPTL_ENABLE_UDP
    {
        if (addr != NULL) { // Respond to given addr and port (used for multicast)
            ...
            r = socketSendTo(gXcpTl.Socket, data, size, gXcpTl.MasterAddr, gXcpTl.MasterPort, NULL);
        }
    }
#endif // UDP
...
static bool XcpEthTlSendV(tQueueBuffer buffers[], uint16_t count) {
...
#ifdef XCPTL_ENABLE_UDP
    {
        ...
        r = socketSendToV(gXcpTl.Socket, buffers, count, gXcpTl.MasterAddr, gXcpTl.MasterPort);
    }
#endif // UDP
...
bool XcpEthTlHandleCommands(void) {
    ...
    if (isTCP()) {
        ...
        n = socketRecv(gXcpTl.Socket, (uint8_t *)&msgBuf.dlc, (uint16_t)XCPTL_TRANSPORT_LAYER_HEADER_SIZE,
        ...
        if (n == XCPTL_TRANSPORT_LAYER_HEADER_SIZE) {
            ...
            if (n > 0) {
                if (n == msgBuf.dlc) {
                    ...
                    socketShutdown(gXcpTl.Socket); // Let the receive thread terminate without error message
...
bool XcpEthTlInit(const uint8_t *addr, uint16_t port, bool useTCP, tQueueHandle Queue) {
    ...
    if (useTCP) { // TCP
        if (!socketOpen(&gXcpTl.ListenSocket, SOCKET_MODE_TCP | SOCKET_MODE_BLOCKING | SOCKET_MODE_REUSEADDR))
            return false;
        if (!socketBind(gXcpTl.ListenSocket, bind_addr, gXcpTl.ServerPort))
            return false;
        if (!socketListen(gXcpTl.ListenSocket))
...
#endif
    { // UDP
        if (!socketOpen(&gXcpTl.Socket, SOCKET_MODE_BLOCKING | SOCKET_MODE_REUSEADDR))
            return false;
    ...
    if (!socketOpen(&gXcpTl.MulticastSock, SOCKET_MODE_BLOCKING | SOCKET_MODE_REUSEADDR))
        return false;
...
#ifdef XCPTL_ENABLE_TCP
    if (isTCP())
        socketClose(&gXcpTl.ListenSocket);
#endif
    socketClose(&gXcpTl.Socket);
...
int32_t XcpTlHandleTransmitQueue(void) {
    ...
    // Collect queue buffers for a segment, until the segment is full or there is no more buffer in the queue
    ...
    for (;;) {
        ...
        if (l == 0) {
            break; // queue is empty, break loop and transmit collected buffers
        } else {

            assert(l <= XCPTL_MAX_CTO_SIZE + 4);
            assert(l % 4 == 0);

            // Check if this buffer fits into the current segment, if not send the current segment and start a new one
            if (length + l > XCPTL_MAX_SEGMENT_SIZE) {
                break; // queue is empty, break loop and transmit collected buffers
            }

            queue_buffers[index] = queue_buffer; // Store buffer for later transmission and release
            index++;
            length += l;
        }
    }
    ...
    // Update the transport layer header for all collected messages
    for (uint32_t i = 0; i < index; i++) {
        ...
        assert(l % 4 == 0);
        *(uint32_t *)queue_buffers[i].buffer = ((uint32_t)XcpTlGetCtr() << 16) | l;
    }
